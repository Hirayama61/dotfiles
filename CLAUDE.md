...
│   ├── sensitive-words.txt.example # 機密語コミットブロックのリスト雛形(各自 cp)
│   └── .coderabbit.yaml.example   # CodeRabbit レビュー設定の雛形(運用先リポへ各自 cp)
├── bin/sync.sh                    # 削除自動化付き chezmoi apply ラッパー
├── bin/skills-sync.sh             # ext-skills.txt の外部 skill を ghq clone & symlink
...
CodeRabbit AI レビューの設定 `.coderabbit.yaml` の **配布用雛形** を `template/.coderabbit.yaml.example`
として持つ。`sensitive-words.txt.example` と同じ「テンプレを commit / 実体は各自 cp」パターン。

- **dotfiles リポ専用設定ではなく、他リポへ配る出発点**。CodeRabbit を効かせたいリポの
  **ルート直下**に置いて使う。dotfiles の chezmoi/apply 対象ではない(`$HOME` 配下でないため自動配置しない)。
- 初期配置: 運用先リポのルートで `cp template/.coderabbit.yaml.example .coderabbit.yaml`
  してリポに合わせて調整する(`.coderabbit.yaml` 実体は各リポ側で管理)。
- 雛形の方針: 言語 `ja` / tone `assertive` / draft PR もレビュー / 全 base ブランチ対象 / poem 無効。
  他リポの知見を横断参照する `knowledge_base.linked_repositories` はリポごとに関連先が違うため
  コメントアウト例(`Hirayama61/cc-dotfiles`)に留め、有効化は各自に委ねる。
