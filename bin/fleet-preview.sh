...
#
# 読み取り専用: 壊れ JSON・読取中に消えたファイル(glob→mv race)は黙って skip し
# stderr に警告 1 行(fail-open)。隔離・修復は home 側の専権で、ここでは行わない。
#
...
環境変数:
  ...
  FLEET_PREVIEW_INTERVAL  再描画間隔秒(既定 2)
...
# 経過時間の人間表示(bash 3.2 互換。date -j は BSD date)。
age_of() {
  # $1 = ISO8601 (JSON の updated_at)。パース不能は "?" を返す(fail-open)。
...
# 1 status 分のタスク行を描画する。$1 = status、$2 = マーク、$3 = 色コード("" で無色)。
render_status() {
  ...
  for f in "$FLEET_DIR"/tasks/*.json; do
    [ -e "$f" ] || continue
    # 読取中の mv/削除(race)や壊れ JSON は skip + stderr 警告(fail-open。隔離は home 専権)
    row="$(jq -r 'select(.status == "'"$status"'") |
      [.repo, .title, .window_name, .next_action, (.context_pct | tostring), .updated_at]
      | @tsv' "$f" 2>/dev/null)" || {
      echo "$PROG: 読めない JSON を skip: $f" >&2
      continue
    }
    [ -n "$row" ] || continue
    IFS="$(printf '\t')" read -r repo title wname naction ctx updated <<EOF2
$row
EOF2
...
render() {
  ...
  render_status waiting-human '!' "$(printf '\033[31m')"
  render_status running '>' ''
  render_status blocked 'x' "$(printf '\033[33m')"
  render_status backlog '.' "$(printf '\033[2m')"
}
