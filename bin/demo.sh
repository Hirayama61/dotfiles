...
# ② Claude Code そっくり静的モック。
#
# 実 claude は起動しない(トークン/ネット不要・再現性 100%)。Claude Code の
# 実画面 chrome を忠実再現する:
#   - 会話域: ユーザ発言は `>` 行、アシスタントは `⏺` バレット、ツール実行は
#     `⏺ Tool(args)`、結果は `  ⎿  …` インデント(実物の体裁)。
#   - 下部: 角丸入力ボックス ╭─ │ ╰─ + ステータス行(モデル名/モード/
#     コンテキスト残量の **ダミー** 表記。実トークン値・実情報は出さない)。
# 色は全て Panda 単一ソース($FG_* は .chezmoidata.yaml 由来)。新規配色なし。
# 題材は claude-transcript.txt と同じ無害なパレットモジュール refactor。
#
# 罫線などの装飾文字は環境非依存にするため box-drawing を直接使う。
render_claude_mock() {
  local dst="$1"
  local DIM="$FG_SUBTLE" UB="$FG_MINT" AB="$FG_MINT" TXT="$FG_FG"
  local TOOL="$FG_CYAN" OKC="$FG_MINT" ACC="$FG_ORANGE" R="$RESET"
  # 入力ボックスの内側幅(罫線含めた見た目幅。狭めの上段右ペインに収まる値)
  local W=58 i bar=""
  for ((i = 0; i < W; i++)); do bar+="─"; done

  {
    # ── ヘッダ(Claude Code 起動バナー風) ──
    printf '%s✻%s %sWelcome to %sClaude Code%s\n' \
      "$ACC" "$R" "$DIM" "$TXT" "$R"
    printf '\n'
    printf '%s  cwd: %s~/dev/palette-engine%s\n' "$DIM" "$DIM" "$R"
    printf '\n'

    # ── 会話域 ──
    printf '%s> %sRefactor the palette module so role lookups are O(1)%s\n' \
      "$UB" "$UB" "$R"
    printf '%s  and the parser never throws. Keep it dependency-free.%s\n' \
      "$UB" "$R"
    printf '\n'

    printf '%s⏺%s %sI'\''ll branch the type model first, then make lookups%s\n' \
      "$AB" "$R" "$TXT" "$R"
    printf '%s  map-backed and return a typed Result instead of%s\n' \
      "$TXT" "$R"
    printf '%s  throwing.%s\n' "$TXT" "$R"
    printf '\n'

    printf '%s⏺%s %sRead%s%s(src/palette.ts)%s\n' \
      "$AB" "$R" "$TOOL" "$R" "$DIM" "$R"
    printf '%s  ⎿  Read 148 lines%s\n' "$DIM" "$R"
    printf '\n'

    printf '%s⏺%s %sUpdate%s%s(src/palette.ts)%s\n' \
      "$AB" "$R" "$TOOL" "$R" "$DIM" "$R"
    printf '%s  ⎿  Updated src/palette.ts with 4 edits%s\n' "$DIM" "$R"
    printf '%s     %s+ type Channel = number & { __brand: "Channel" }%s\n' \
      "$DIM" "$OKC" "$R"
    printf '%s     %s+ function parseHex(hex): Result<Rgb>%s\n' \
      "$DIM" "$OKC" "$R"
    printf '%s     %s+ class Palette<Roles extends Role = Role>%s\n' \
      "$DIM" "$OKC" "$R"
    printf '\n'

    printf '%s⏺%s %sBash%s%s(node --test palette)%s\n' \
      "$AB" "$R" "$TOOL" "$R" "$DIM" "$R"
    printf '%s  ⎿  %s✔%s 4 passing  (11 ms)%s\n' "$DIM" "$OKC" "$DIM" "$R"
    printf '\n'

    printf '%s⏺%s %sLookups are now map-backed (O(1)). parseHex returns%s\n' \
      "$AB" "$R" "$TXT" "$R"
    printf '%s  a typed Result, and contrastText keeps labels%s\n' \
      "$TXT" "$R"
    printf '%s  readable on bright accents. No new dependencies.%s\n' \
      "$TXT" "$R"
    printf '\n'
    printf '\n'

    # ── 入力ボックス(角丸 box-drawing) ──
    printf '%s╭%s%s╮%s\n' "$DIM" "$bar" "$DIM" "$R"
    printf '%s│%s %s>%s Ship it.%*s%s│%s\n' \
      "$DIM" "$R" "$UB" "$R" 47 "" "$DIM" "$R"
    printf '%s╰%s%s╯%s\n' "$DIM" "$bar" "$DIM" "$R"

    # ── ステータス行(全部ダミー。実トークン値・実情報なし) ──
    printf '%s  %s⏵⏵%s accept edits on   %sclaude%s   %scontext left: ▓▓▓▓▓░░%s\n' \
      "$DIM" "$OKC" "$DIM" "$ACC" "$DIM" "$DIM" "$R"
  } > "$dst"
}
